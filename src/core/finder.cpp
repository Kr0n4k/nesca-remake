...
int sharedDetector(const char * ip, int port, const std::string *buffcpy, const char *cp) {
	...
	// Extended fingerprint scanning - check ONVIF, RTSP streams, API endpoints, SNMP
	// Only if we have a response buffer or specific ports
	if (port == 80 || port == 8080 || port == 554 || port == 8554 || port == 161) {
		FingerprintResult fpResult = FingerprintScanner::scanDevice(ip, port);
		
		if (fpResult.detected) {
			if (fpResult.deviceType == "onvif") {
				return 100; // ONVIF device
			} else if (fpResult.deviceType == "rtsp_stream") {
				return 101; // RTSP stream detected
			} else if (fpResult.deviceType == "api_camera") {
				// Map to appropriate return code based on manufacturer
				if (fpResult.manufacturer == "Hikvision") return 34;
				else if (fpResult.manufacturer == "Dahua") return 102; // Dahua API
				else if (fpResult.manufacturer == "Axis") return 20;
				else if (fpResult.manufacturer == "Panasonic") return 25;
				else if (fpResult.manufacturer == "Sony") return 26;
	...
	// Check for ONVIF in response body
	if (Utils::ustrstr(buffcpy, "onvif") != -1 ||
		Utils::ustrstr(buffcpy, "device_service") != -1 ||
		Utils::ustrstr(buffcpy, "xmlns:tds") != -1) {
		return 100; // ONVIF device
	}
